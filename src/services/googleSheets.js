...
class GoogleSheetsService {
    ...
    async initialize() {
        ...
        if (!process.env.GOOGLE_SHEETS_CLIENT_ID || !process.env.GOOGLE_SHEETS_CLIENT_SECRET || !process.env.GOOGLE_SHEETS_REFRESH_TOKEN) {
            console.log('⚠️ Google Sheets credentials not configured. Service will be disabled.');
            this.disabled = true;
            return;
        }
        ...
        try {
            ...
            try {
                ...
                console.warn('Warning: Could not initialize Google Sheets headers:', headerError.message);
                console.log('Google Sheets service will work without header initialization');
            }

            console.log('✅ Google Sheets service initialized successfully');
        } catch (error) {
            console.error('❌ Error initializing Google Sheets service:', error.message);
            console.log('⚠️ Google Sheets service will be disabled. Video processing will continue without tracking.');
            this.disabled = true;
        }
    }
    ...
    async addVideoRecord(data) {
        if (this.disabled) {
            console.log('Google Sheets service is disabled. Skipping record addition.');
            return false;
        }
        ...
        try {
            ...
            console.log('Continuing without Google Sheets tracking...');
            return false;
        }
    }
